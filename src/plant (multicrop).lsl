...
// Changed notecard so now you have setting for SF_PREFIX and don't use it for product names e.g.  was 'SF Wood' now is 'Wood'

//### plant.lsl
/**
This script is common for trees, plant fields, grapes etc.  It supports multiple plants.
To add a new text based plant, e.g. Mango:
...
5. Set the name of the prim that is the plant texture to 'Foliage'
OR
To add a new prim based plant, e.g. Mushroom:
/ Prim name can be one or more of New, Growing and Ripe  e.g 'Ripe' or 'New;Growing;Ripe' etc
  Prims with Growing in their name grow if growPrims is TRUE
#
**/
float  VERSION = 4.5;    // Beta 24 November 2020
integer RSTATE = 0;      // RSTATE = 0 for release, 0 for beta, -1 for Release candidate
...
list BY_PRODUCTS = [];                      // BY_PRODUCTS=Leaves,Twigs,Branches,Logs      Items produced during the growing cycle - can use this as well as or in place of 'WOOD' settings
list BY_PRODUCT_TIMES = [];                 // BY_PRODUCT_TIMES=4,3,2,1                    how many times to give by-product during the growing phase

integer HAS_WOOD = 0;                       //  HAS_WOOD=0                                 whether this plant gives wood. If yes, the SF Wood object must be in the contents)
float WOOD_TIMES = 2.0;                     //  WOOD_TIMES=2                               how many times to give wood during the growing phase
string WOOD_OBJECT = "Wood";                //  WOOD_OBJECT=Wood                           What to rez as 'wood' product

list PLANTS = [];                           //  PLANTLIST=Orange Tree,Apple Tree etc       names of supported plants, separated with comma, No SF prefix
list PRODUCTS = [];                         //  PRODUCTLIST=SF Oranges,SF Apples etc       names of products each plant rezzes, with SF prefix
...
string  SF_WATER="Water";                   //  WATER_OBJECT=Water                         Name of water object to accept for manual watering
string  SF_MANURE="Manure";                 //  MANURE_OBJECT=Manure                       Name of object to accept as manure
string  SF_COMPOSTABLE="Compostable";       //  COMPOSTABLE_OBJECT                         Name of item to give as compostable product
...
string  TXT_GIVE_WOOD="Here is your pile of wood";
string  TXT_FOUND_WATER="Found water. how refreshing!";
...
string  TXT_STATUS_WATER="Water:";
string  TXT_STATUS_WOOD="Wood:";
string  TXT_LOW_ENERGY="Not enough energy for task";
string  TXT_ERROR_UPDATE="Error: unable to update - you are not my Owner";
string  TXT_ERROR_LOCKED="Error: I am locked, did you try to copy me?\nTo unlock me without losing any progress, just ask the farm manager";
...
loadConfig(integer checkForReset)
{
    ...
    if (llList2String(desc, 0) == "T")
    {
        ...
        else
        {
            ...
            if (llList2String(desc, 10) != "") byProdLevels = llParseString2List(llList2String(desc, 10), [";"], []);
        }
    }
}
...
save2Desc()
{
    string codedDesc = "T;"+PRODUCT_NAME+";"+status+";"+(string)(statusLeft)+";"+(string)llRound(water)+";"+(string)llRound(wood)+";"+plant+";"+(string)chan(llGetKey())+";"+(string)autoWater + ";"+languageCode;
        ...
        for (i=0; i < llGetListLength(lines); i++)
        {
            ...
            if (llGetSubString(line, 0, 0) != "#")
            {
                ...
                if (llList2String(tok,1) != "")
                {
                    ...
                    else if (cmd == "TXT_GIVE_WOOD") TXT_GIVE_WOOD = val;
...
string setByProdLevels()
{
    ...
    if (count >0)
    {
        ...
        for (index = 0; index < count; index++)
        {
            ...
            if (result >= 100.0) bpText += llList2String(BY_PRODUCTS, index)+"\n"; else progress += llList2String(BY_PRODUCTS, index) +": " +(string)(llFloor(llList2Float(byProdLevels, index)))+"%\n";
        }
        if (bpText != TXT_RIPE+":\n") llMessageLinked(LINK_SET, 1, "BP_READY|"+bpText, "");
    }
    return progress;
}
...
refresh()
{
    ...
    if (status == "New" || status == "Growing" || status == "Ripe")
    {
        ...
        float p = 1- ((float)(statusLeft)/(float)statusDur);
             ...
             if (status == "Empty") progress += TXT_STATUS+TXT_EMPTY;
        else if (status == "New") progress += TXT_STATUS+TXT_NEW;
        else if (status == "Growing") progress += TXT_STATUS+TXT_GROWING;
        else if (status == "Ripe") progress += TXT_STATUS+TXT_RIPE;
        else if (status == "Dead") progress += TXT_STATUS+TXT_DEAD;
        progress += " ("+(string)((integer)(p*100.))+"%)\n";
        progress += TXT_STATUS_WATER + (string)((integer)osMax(0.0, water))+ "%\n";
        if (HAS_WOOD)
        {
            progress += TXT_STATUS_WOOD+(string)(llFloor(wood))+"%\n";
        }
...
doHarvest()
{
    if (status == "Ripe")
    {
        //llRezObject(PRODUCT_NAME, llGetPos() + rezzPosition*llGetRot() , ZERO_VECTOR, ZERO_ROTATION, 1);
...
default
{
    ...
    touch_start(integer n)
    {
        ...
        {
            result = llList2Float(byProdLevels, index);
            if (result >= 100.0) opts += llList2String(BY_PRODUCTS, index);
        }
        //
    ...
    listen(integer c, string n ,key id , string m)
    {
        ...
        else if ((m == TXT_WATER_PLANT) || (m == NPC_WATER_PLANT))
        {
            ...
            {
                llSensor(SF_PREFIX+SF_WATER, "", SCRIPTED, 5, PI);
            }
            else
            {
        ...
        else if (m == TXT_ADD_MANURE)
        {
            if ((energy >= 1) || (energy == -1))
            {
                llSensor(SF_PREFIX+SF_MANURE, "", SCRIPTED, 5, PI);
            }
            else
            {
                llRegionSayTo(lastUser, 0, TXT_LOW_ENERGY);
        ...
        else if (m == TXT_CLEANUP)
        {
            ...
            {
                setFoliage(FALSE, ZERO_VECTOR);
                llMessageLinked(LINK_SET, 0, "REZ_PRODUCT|" +PASSWORD +"|" +(string)lastUser +"|" +SF_PREFIX+SF_COMPOSTABLE, NULL_KEY);
                status="Empty";
        ...
        else if (m == TXT_GET_WOOD)
        {
            if (wood >=100.)
            {
                llMessageLinked(LINK_SET, 0, "REZ_PRODUCT|" +PASSWORD +"|" +(string)lastUser +"|" +SF_PREFIX+WOOD_OBJECT, NULL_KEY);
                wood =0;
                refresh();
                llTriggerSound("lap", 1.0);
                llRegionSayTo(lastUser, 0, TXT_GIVE_WOOD);
                llRegionSay(FARM_CHANNEL, "HEALTH|" +PASSWORD+"|" +(string)lastUser +"|Hygiene|-1|Health|2");
            }
        }
        else if (llGetListLength(BY_PRODUCTS) != 0)
        {
            integer index;
            integer count = llGetListLength(BY_PRODUCTS);
            for (index = 0; index < count; index++)
            {
                if (m == llList2String(BY_PRODUCTS, index))
                {
                    llMessageLinked(LINK_SET, 0, "REZ_PRODUCT|" +PASSWORD +"|" +(string)lastUser +"|" +SF_PREFIX+m, NULL_KEY);
