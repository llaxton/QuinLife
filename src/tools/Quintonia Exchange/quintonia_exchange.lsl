string TXT_MEDIEVAL = "Medieval";
string TXT_GOREAN = "Gorean";

string    TXT_INSUFFICIENT_POINTS  =  "Sorry, you don't have enough coins.";
...
// Added support for "WAS_TOUCHED" link message
//
/////////////////////////////////////////////
...
string  exchangeType = "grocery";           // TYPE=Grocery         can be 'grocery', 'bazaar', 'hardware', 'concessions', medieval, gorean [bazaar is grocery minus concessions items]
...
list   markets       = ["grocery", "bazaar", "hardware", "concessions", "medieval", "gorean"];
string PASSWORD      = "*";
...
loadConfig()
{
    ...
    if (llListFindList(markets, exchangeType) != -1) marketType = "market_"+exchangeType+"inf"; else marketType = "market_grocery.inf";
        ...
        for (i=0; i < llGetListLength(lines); i++)
        {
            ...
            if (llGetSubString(line, 0, 0) != "#")
            {
                ...
                if (llList2String(tok,1) != "")
                {
                    ...
                    else if (cmd == "TXT_MEDIEVAL")          TXT_MEDIEVAL         = val;
...
doTouch(key avatarID)
{
    ...
    {
        opts += [TXT_COINS];
        if (useScreen == TRUE) opts += [TXT_PRICES];
        opts += [TXT_CLOSE, TXT_SELL, TXT_BUY, TXT_POINTS, TXT_LANGUAGE];
    }
    if (userToPay == owner)
    {
