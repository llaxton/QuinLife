...
integer dropTime = 86400;                   // DROP_TIME=2
...
refresh(integer didChange)
{
    ...
    if (didChange == TRUE)
    {
        ...
        for (i=0; i < productsLength; i++)
        {
            ...
            for (lnk=2; lnk <= count; lnk++)
            {
                ...
                if (llGetLinkName(lnk) == product)
                {
                    ...
                    if (TXT_COLOR != ZERO_VECTOR) llSetLinkPrimitiveParamsFast(lnk, [PRIM_TEXT, stati, c, 1.0]);
        ...
        if (llGetListLength(noPrimProducts) != 0)
        {
            ...
            {
                llSetLinkPrimitiveParamsFast(LINK_THIS, [PRIM_TEXT,  statTotal +customStr, TXT_COLOR, 1.0]);
            }
            else
            {
        ...
        {
            llSetLinkPrimitiveParamsFast(LINK_THIS, [PRIM_TEXT, customStr, TXT_COLOR, 1.0]);
        }
